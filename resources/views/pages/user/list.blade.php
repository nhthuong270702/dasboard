          ...
          <div class="row">
              <div class="col-md-12">
                  <div class="add" style="margin: 20px 0">
                    <button class="btn btn-primary"><a style="color:aliceblue;" href="{{ route('user.create') }}">Add User</a></button>
                     ...
                     <tbody>
                         ...
                         <tr id="sid{{$al->id}}">
                            <td><input type="checkbox" name="ids" class="checkBoxClass" value="{{$al->id}}"></td>
                             ...
                             <td>{{$al->password}}</td>
                             <td>
                                <form method="POST" action="{{ route('user.delete', $al->id) }}">
                                    @csrf
                         ...
                         @endforeach
                         <a href="{{ route('user.restore')}}">Khôi phục các người dùng đã xóa</a>
                     </tbody>
                 </table>
              </div>
          </div>
